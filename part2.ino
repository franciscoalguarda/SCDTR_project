...
float current_lux = 0;           
float current_u = 0;       
char current_occupancy = 'h'; // 'o' (off), 'l' (low) or 'h' (high)       

// --- PHASE 2 VARIABLES (TABLE 3) ---
float lower_bound_low = 10.0;  // U
float lower_bound_high = 50.0; // O
float setpoint = lower_bound_high; // Current reference in lux
...
void setup() {
  ...
  // Check CAN bitrate setting
  if (mcp2515->setBitrate(CAN_500KBPS, MCP_8MHZ) != MCP2515::ERROR_OK) {
    Serial.println("ERROR: CAN bitrate setting failed!");
    while(1); // Halt execution
  }
  
  // Check CAN normal mode setting
  if (mcp2515->setNormalMode() != MCP2515::ERROR_OK) {
    Serial.println("ERROR: CAN normal mode setting failed!");
    while(1); // Halt execution
  }
...
float getFilteredLux(float new_sample) {
  if (new_sample < 0) {
    Serial.println("ERROR: Invalid sample for filtering!");
    return current_lux; // Return last valid reading
  }
...
float getLux() {
  ...
  if (R_LDR <= 0) {
    Serial.println("ERROR: Invalid LDR resistance calculation!");
    return -1.0; // Return error value
  }
  
  float logR = log10(R_LDR);
  float lux = pow(10, (logR - b) / m);
  
  // Validate lux value is reasonable (not negative, not infinite)
  if (lux < 0 || !isfinite(lux)) {
    Serial.println("ERROR: Invalid lux calculation!");
    return -1.0; // Return error value
  }
...
void calibrateGain() {
  ...
  for(int j = 0; j < 50; j++) {
    ...
    if (val >= 0) {
      ...
      Serial.println("WARNING: Invalid reading during calibration");
    }
  ...
  if (valid_readings > 0) {
    background_lux = sum / valid_readings;
    Serial.print("Background LUX Calibrated: ");
    Serial.println(background_lux);
  } else {
    Serial.println("ERROR: No valid readings during calibration!");
    background_lux = 0.0; // Safe default
  }
}
...
void sendCANNetworkMsg(char action, char variable, int dest_node, float value) {
  ...
  if (result != MCP2515::ERROR_OK) {
    Serial.print("ERROR: CAN send failed with code ");
...
void readCANMessages() {
  ...
  if (result == MCP2515::ERROR_OK) {
    ...
    if (sender_id >= 1 && sender_id <= MAX_NODES && variable == 'y') {
        if (sender_id - 1 >= 0 && sender_id - 1 < MAX_NODES) {
            network_lux[sender_id - 1] = value;
        } else {
            Serial.println("ERROR: Invalid sender_id for network_lux array!");
        }
    }
    ...
    // Only report errors that are not "no message" (which is normal)
    Serial.print("ERROR: CAN read failed with code ");
    Serial.println(result);
  }
}
...
void processUI() {
  if (Serial.available() > 0) {
    ...
    if (cmd == 'g') {
      if (input.length() < 4) {
        Serial.println("ERROR: Invalid 'g' command format!");
        return;
      }
      char subcmd = input.charAt(2);
      int target_node = input.substring(4).toInt(); 
      
      if (target_node < 1 || target_node > MAX_NODES) {
        Serial.println("ERROR: Invalid target node!");
        return;
      } 
    ...
    if (secondSpace != -1) {
      ...
      // Note: val validation depends on the specific command
...
void loop() {
  ...
  if (millis() - last_t >= 10) { 
    ...
    float raw_lux = getLux();
    if (raw_lux >= 0) {
      current_lux = getFilteredLux(raw_lux);
    } else {
      Serial.println("WARNING: Using previous lux reading due to sensor error");
      // Keep previous current_lux value
    }
    ...
    if (feedback_enabled) {
      ...
      // Validate PID output
      if (current_u < 0 || current_u > 4095 || !isfinite(current_u)) {
        Serial.print("ERROR: Invalid PID output: ");
        Serial.println(current_u);
        current_u = 0; // Safe fallback
      }
    } else {
      current_u = manual_pwm;
      // Validate manual PWM
      if (current_u < 0 || current_u > 4095) {
        Serial.print("ERROR: Invalid manual PWM value: ");
        Serial.println(current_u);
        current_u = 0; // Safe fallback
      }
    }
    ...
    if (history_counter >= 10) {
      if (history_idx >= 0 && history_idx < minute_buffer_size) {
        history_y[history_idx] = current_lux;
        history_u[history_idx] = current_u;
        history_idx = (history_idx + 1) % minute_buffer_size;
      } else {
        Serial.println("ERROR: Invalid history_idx!");
        history_idx = 0; // Reset to safe value
      }
